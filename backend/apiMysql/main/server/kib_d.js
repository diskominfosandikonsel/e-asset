...
router.post('/view', (req, res) => {
    ...
    if (!tahun) {
        ...
        filterTahun = ` AND YEAR(kib_d.tgl_beli) = '` + tahun + `'`
    }


    let jml_data = `
        SELECT 
        kib_d.*, hak.uraian as hak_tanah, asal.uraian as asal_usul

        FROM e_aset.kib_d kib_d 

        LEFT JOIN e_aset.master_hak hak
        ON hak.id = kib_d.id_hak

        LEFT JOIN e_aset.master_asal asal
        ON asal.id = kib_d.id_asal

        WHERE 
        kib_d.unitId = '`+req.user.profile.unit_kerja+`' AND
        kib_d.keterangan LIKE '%`+cari+`%'
        `+filterTahun+` 
        ORDER BY kib_d.createAt DESC
    `
...
router.post('/admin', (req, res) => {
    ...
    if (!unit) {
        ...
        filterUnit = ` AND kib_d.unitId = '`+unit+`'`
    }
...
router.post('/addData', upload.single("file"), (req,res)=>{
    ...
    insert = `INSERT INTO kib_d (id, kodep, kodea, tgl_beli, tgl_buku, luas, alamat, id_hak, no_sert, tgl_sert, id_asal, guna, harga, keterangan, file, unitId, userId, createAt) 
...
router.post('/removeData', (req, res)=> {
    ...
    hapus_file(file);
    var query = `
        DELETE FROM kib_d WHERE id = '`+req.body.id+`'; 
    `;
    db.query(query, (err, row)=>{
        if(err){
            res.send(err);
