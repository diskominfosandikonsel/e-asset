...
router.post('/view', (req, res) => {
    ...
    if (!tahun) {
        ...
        filterTahun = ` AND YEAR(kib_e.tgl_beli) = '` + tahun + `'`
    }


    let jml_data = `
        SELECT 
        kib_e.*, hak.uraian as hak_tanah, asal.uraian as asal_usul

        FROM e_aset.kib_e kib_e 

        LEFT JOIN e_aset.master_hak hak
        ON hak.id = kib_e.id_hak

        LEFT JOIN e_aset.master_asal asal
        ON asal.id = kib_e.id_asal

        WHERE 
        kib_e.unitId = '`+req.user.profile.unit_kerja+`' AND
        kib_e.keterangan LIKE '%`+cari+`%'
        `+filterTahun+` 
        ORDER BY kib_e.createAt DESC
    `
...
router.post('/admin', (req, res) => {
    ...
    if (!unit) {
        ...
        filterUnit = ` AND kib_e.unitId = '`+unit+`'`
    }
...
router.post('/addData', upload.single("file"), (req,res)=>{
    ...
    insert = `INSERT INTO kib_e (id, kodep, kodea, tgl_beli, tgl_buku, luas, alamat, id_hak, no_sert, tgl_sert, id_asal, guna, harga, keterangan, file, unitId, userId, createAt) 
...
router.post('/removeData', (req, res)=> {
    ...
    hapus_file(file);
    var query = `
        DELETE FROM kib_e WHERE id = '`+req.body.id+`'; 
    `;
    db.query(query, (err, row)=>{
        if(err){
            res.send(err);
