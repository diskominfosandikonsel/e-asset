...
router.post('/view', (req, res) => {
    ...
    if (!tahun) {
        ...
        filterTahun = ` AND YEAR(kib_f.tgl_beli) = '` + tahun + `'`
    }


    let jml_data = `
        SELECT 
        kib_f.*, hak.uraian as hak_tanah, asal.uraian as asal_usul

        FROM e_aset.kib_f kib_f 

        LEFT JOIN e_aset.master_hak hak
        ON hak.id = kib_f.id_hak

        LEFT JOIN e_aset.master_asal asal
        ON asal.id = kib_f.id_asal

        WHERE 
        kib_f.unitId = '`+req.user.profile.unit_kerja+`' AND
        kib_f.keterangan LIKE '%`+cari+`%'
        `+filterTahun+` 
        ORDER BY kib_f.createAt DESC
    `
...
router.post('/admin', (req, res) => {
    ...
    if (!unit) {
        ...
        filterUnit = ` AND kib_f.unitId = '`+unit+`'`
    }
...
router.post('/addData', upload.single("file"), (req,res)=>{
    ...
    insert = `INSERT INTO kib_f (id, kodep, kodea, tgl_beli, tgl_buku, luas, alamat, id_hak, no_sert, tgl_sert, id_asal, guna, harga, keterangan, file, unitId, userId, createAt) 
...
router.post('/removeData', (req, res)=> {
    ...
    hapus_file(file);
    var query = `
        DELETE FROM kib_f WHERE id = '`+req.body.id+`'; 
    `;
    db.query(query, (err, row)=>{
        if(err){
            res.send(err);
